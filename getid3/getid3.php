...
{
	// public: Settings
	var $encoding        = 'UTF-8'; //ISO-8859-1';   // CASE SENSITIVE! - i.e. (must be supported by iconv())
	...
	function getID3()
	{
		...
		if (eregi('([0-9]+[ ]?)M', $memory_limit, $matches)) {
			// could be stored as "16M" or "16 M" rather than 16777216 for example
		...
		if (!defined('GETID3_HELPERAPPSDIR')) {
			define('GETID3_HELPERAPPSDIR', realpath(GETID3_INCLUDEPATH.'helperapps').DIRECTORY_SEPARATOR); // must not have any space in this path
		}
